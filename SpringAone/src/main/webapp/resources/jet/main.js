...
require([ 'ojs/ojcore', 'knockout', 'jquery', 'ojs/ojknockout', 'ojs/ojrouter', 'ojs/ojmodule', 'ojs/ojoffcanvas', 'ojs/ojnavigationlist',
	...
	function RootViewModel() {
		...
		// Shared navigation data and callbacks for nav bar (medium+ screens)
		// and nav list (small screens)
		var navData = [ {
			name : 'Browse Stuff',
			id : 'browse',
		...
		self.navChange = function(event, ui) {
			if (ui.option === 'selection' && ui.value !== self.router.stateId()) {
				// Only toggle navigation drawer when it's shown on small
				// screens
				if (self.smScreen())
					self.toggleDrawer();
		...
		// Called by navigation drawer toggle button and after selection of nav
		// drawer item
